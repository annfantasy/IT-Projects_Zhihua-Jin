...
gt = .1 * (np.random.rand(length) - .5)

def fitness(sol):
    '''
    inner product with the gt
    '''
    return np.sum(gt * sol)
...
if __name__ == '__main__':
    print('maximal fitness in groundtruth: %.3f'%np.sum(gt[gt > 0]))
    ...
    for i in range(testing_times):
        ...
        t1 = time.time()
        _, fit_ga[i], total_iter_ga[i] = genetic_algorithm(fitness, neighbor_generator, init_generator, mutator, group_ratio=.2)
        t2 = time.time()
