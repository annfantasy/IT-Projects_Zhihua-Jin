<template>
  <div class="app">
    ...
    <mu-bottom-nav :value.sync="active" class="bottom-nav" shift>
      ...
      <span v-if="account.role === 0" @click="healthClick">
        <mu-bottom-nav-item value="health" title="Health" icon="favorite"/>
      </span>
      <span v-if="account.role === 1" @click="scheduleClick">
        <mu-bottom-nav-item value="schedule" title="Schedule" icon="today"/>
      </span>
      <span v-if="account.role === 0" @click="dataClick">
...
export default {
  ...
  methods: {
    ...
    scheduleClick() {
      this.$router.replace('/schedule/index')
    },
    dataClick() {
